load('workspace_between')
%load('workspace_no_between')
%%
...
X(rmv, :) = [];
[~, ~, variable_names] = preprocess_input(X);
%
%%
[a, p_idx] = sort(r_mean, 'descend');
p_first_idx = find(a==r_mean(1))
%%
figure, hold on ;histogram(mse_(:, 1))
%%
for ii = 2:size(r, 1)
...
h3b = histogram(ifisherz(r(p_idx(1),:)));
title('Compare correlations on many partitions')
...
h4b = histogram(ifisherz(r(p_idx(end),:)));
h4a.EdgeColor = 'w';
h4b.EdgeColor = 'w';
h4a.FaceColor = myC1;
h4b.FaceColor = myC2;
...
text(-0.65, 0.06, sprintf('p-value = %.2f', p_first_idx / size(r_mean, 1)), 'fontsize', 6);
